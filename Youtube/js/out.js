...
$.fn.ycp = function(j) {
    const n = {
        ...
        col : 2,
        groups: [
            {title: 'name', id: 'PLdhrcCVXurgL2ITXpFIGpXpJfYOhpA_AC'},
            {title: 'name', id: 'UC4F-DMVCe-UbIsweL6kIEfg'},
        ]
    };
    ...
    j.groups = (j.groups == undefined ? n.groups : j.groups);
    ...
    $(this).each(function(i, a) {
        ...
        const html = `${yqa_group_nav()}<div class="ycp yqa-content-feed">
        ...
        console.log(channel);
          if (channel.substring(0, 2) == 'PL' || channel.substring(0, 2) == 'UU') {
            ...
            if($(this).val().length >= 1  ){
                yqa_search($(this).val() ,title, channel, c, i, b);
                console.log(channel);
            }else{
                ycp_list(title, channel, c, i, b);
            }
    ...
    function yqa_group_nav(){
        if(j.groups != ''  || j.groups != undefined){
            html_group = '<div class="yqa-widget-feed-nav"> <div class="yqa-widget-feed-nav-inner">';
            html_group += `<ul class="yqa-widget-feed-nav-list">`;
            $.each( j.groups, function(i, a){
                 html_group += `<li class="yqa-widget-feed-nav-list-item"> <a href="javascript:void(0)" data-yqa-id="${a.id}">${a.title}</a></li> `;   
            });
    ...
    function yqa_group_nav_click(h, k,l){
        if( $(`${l} .yqa-widget-feed-nav`).length  > 0 ) {
            $(`${l} .yqa-widget-feed-nav li.yqa-widget-feed-nav-list-item a:first`).addClass('active');
            $(`${l} .yqa-widget-feed-nav li.yqa-widget-feed-nav-list-item a`).click( function(event){
                $(this).addClass('active');
                $(`${l} .yqa-widget-feed-nav li.yqa-widget-feed-nav-list-item a`).not(this).removeClass('active');
                 if($(this).hasClass('active') == true){
                    let data = $(this).data('yqa-id');
                    if (data.substring(0, 2) == 'PL' || data.substring(0, 2) == 'UU') {
                        const c = '';
                        ycp_list(h, data, c, k, l)
                    } else {
                        const d = (data.substring(0, 2) == 'UC' ? 'id' : 'forUsername');
                        ycp_play(h, data, d, k, l)
                    }                    
                }
            });

            let first_item =  $(`${l} .yqa-widget-feed-nav li.yqa-widget-feed-nav-list-item a:first`);
            if(first_item.hasClass('active') == true){
                let data = first_item.data('yqa-id');
                if (data.substring(0, 2) == 'PL' || data.substring(0, 2) == 'UU') {
                    const c = '';
                    ycp_list(h, data, c, k, l)
                } else {
